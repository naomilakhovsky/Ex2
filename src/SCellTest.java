...
class SCellTest {
    ...
    private int[] parseCellReference(String ref) {
        // ref like "A1", "B2", etc.
        // 1) Extract the column letter(s) and row number
        char colChar = ref.charAt(0);
        String rowPart = ref.substring(1); // e.g. "1" for "A1"

        // 2) Convert column letter 'A'..'Z' -> 0..25 (simple version for 1-letter columns)
        int x = colChar - 'A';  // 'A' => 0, 'B' => 1, etc.

        // 3) Convert row string -> integer (NO subtracting 1 if you want "A1" => y=1)
        int y = Integer.parseInt(rowPart);

        // So "A1" => (x=0, y=1), "B2" => (x=1, y=2), etc.
        return new int[] { x, y };
    }
    ...
    void computeForm() {
        ...
        // Add sample data to the Ex2Sheet using x, y indices
    ...
    void getOrder() {
        ...
        // Initially, the order should be 0 (assuming it's the default value)
        assertEquals(0, cell.getOrder(), "Expected initial order to be 0.");

        // Update the order using reflection (if `setOrder` isn't exposed)
        cell.setOrder(5);
        assertEquals(5, cell.getOrder(), "Expected order to be updated to 5.");

        // Set another order and verify
    ...
    void testToString() {
        ...
        // Check that toString() matches the initial data
        assertEquals("Hello", cell.toString(), "Expected toString to return 'Hello'.");

        // Update the data in the cell and verify
        cell.setData("123");
    ...
    void testSetAndGetType() {
        ...
        // Set type to another valid value
        cell.setType(-1);
        assertEquals(-1, cell.getType(), "Expected type to be -1 after setting.");

        // Set type to a large value
        cell.setType(100);
        assertEquals(100, cell.getType(), "Expected type to be 100 after setting.");

        // Set type to a small negative value
        cell.setType(-100);
        assertEquals(-100, cell.getType(), "Expected type to be -100 after setting.");
    }
